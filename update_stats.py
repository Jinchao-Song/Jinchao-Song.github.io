...
# Jinchao Song's Semantic Scholar Author ID
# Found by searching: https://api.semanticscholar.org/graph/v1/author/search?query=Jinchao+Song
...
def update_html(citations, h_index):
    ...
    # Update citation counter target value in JS
    html = re.sub(
    ...
    # Update displayed h-index (the second stat-num after Citations)
    # Replace the two fixed stat numbers
    html = re.sub(
        r'(<span class="stat-num">)10(</span>\s*<span class="stat-label">h-index)',
        rf'\g<1>{h_index}\2',
        html
    )
    ...
    print(f"index.html updated: citations={citations}, h-index={h_index}")
